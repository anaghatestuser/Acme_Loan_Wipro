...
class FileProcessorAgent(AcmeLoanAgentFramework):
    ...
    async def handle(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            file_summary = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, file_summary, content_type='application/json', variable_name='file_summary', source_file=__file__, before_line=104))
        except _gr_client.GuardrailUnavailableError:
            pass
        except PermissionError:
            raise
