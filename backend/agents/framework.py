...
class AcmeLoanAgentFramework(ABC):
    ...
    async def call_bedrock_model(
        ...
        # LINEAJE: enforce() `messages` at agent->llm pre_model — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_APP_SEC_059 (Do not allow prompts that can execute malicious commands at runtime.); AI_APP_SEC_071 (Enforce chemical, biological, radiological, or nuclear (CBRN) threat prevention safeguards in AI-enabled systems). Mask/block; do not remove without review. site_id='site:sha256:afbc6e312e2340ff620f9ce71c53f5a9dd6087efcbdac2be4c29221b79663e07'
