...
class CreditEvalAgent(AcmeLoanAgentFramework):
    ...
    async def handle(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            safe_combined_context = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, safe_combined_context, content_type='application/json', variable_name='safe_combined_context', source_file=__file__, before_line=106))
        except _gr_client.GuardrailUnavailableError:
            pass
        except PermissionError:
            raise
